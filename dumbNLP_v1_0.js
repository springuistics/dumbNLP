...
function Lemmatize(text){
    ...
    results.forEach(function(word){
        ...
        //lemmatize words with various common endings
                  ...
                  const commonadjs = ['blacker', 'blanker', 'bolder', 'brasher', 'briefer', 'brighter', 'calmer', 'cheaper', 'chiller', 'cleaner', 'clearer', 'crisper', 'cleverer', 'deader', 'deeper', 'defter', 'drunker', 'duller', 'dumber', 'fainter', 'fairer', 'faster', 'firmer', 'flatter', 'fouler', 'franker', 'fuller', 'grander', 'greater', 'greener', 'grosser', 'harder', 'higher', 'hollower', 'iller', 'kinder', 'laxer', 'leaner', 'lesser', 'lighter', 'longer', 'louder', 'lower', 'meaner', 'moister', 'narrower', 'nearer', 'number', 'odder', 'plumper', 'prouder', 'quieter', 'rasher', 'richer', 'righter', 'rounder', 'rougher', 'sharper', 'shorter', 'shyer', 'sicker', 'sleeker', 'slower', 'smoother', 'sourer', 'steeper', 'sterner', 'stiller', 'straighter', 'stricter', 'stupider', 'swifter', 'tenderer', 'tighter', 'warmer', 'weirder', 'wilder', 'yellower', 'younger'];
                  ...
                  const commonadjs3 = ['angrier', 'bloodier', 'bossier', 'busier', 'chewier', 'chubbier', 'classier', 'cloudier', 'clumsier', 'crazier', 'creepier', 'crunchier', 'curlier', 'deadlier', 'dirtier', 'drier', 'earlier', 'easier', 'emptier', 'fancier', 'filthier', 'flakier', 'funnier', 'furrier', 'greasier', 'gloomier', 'greedier', 'healthier', 'heavier', 'hungrier', 'juicier', 'lazier', 'lonelier', 'prettier', 'readier', 'roomier', 'saltier', 'shinier', 'skinnier', 'smellier', 'windier', 'wealthier', 'tidier'];
                  if (commonadjs.includes(word)){
                    possible_lemma = word.slice(0, -2);
                  } else if (word == "bigger" || word == "dimmer" || word == "fitter" || word == "hotter" || word=="fatter" || word == "madder" || word == "sadder" || word == "slimmer" || word == "wetter"){
                    possible_lemma = word.slice(0, -3);
                  ...
                  const commonadjest3 = ['angriest', 'bloodiest', 'bossiest', 'busiest', 'chewiest', 'chubbiest', 'classiest', 'cloudiest', 'clumsiest', 'craziest', 'creepiest', 'crunchiest', 'curliest', 'deadliest', 'dirtiest', 'driest', 'earliest', 'easiest', 'emptiest', 'fanciest', 'filthiest', 'flakiest', 'funniest', 'furriest', 'greasiest', 'gloomiest', 'greediest', 'healthiest', 'heaviest', 'hungriest', 'juiciest', 'laziest', 'loneliest', 'prettiest', 'readiest', 'roomiest', 'saltiest', 'shiniest', 'skinniest', 'smelliest', 'windiest', 'wealthiest','tidiest'];
                  if (commonadjest.includes(word)){
                    possible_lemma = word.slice(0, -3);
                  } else if (word == "biggest" || word == "dimmest" || word == "fittest" || word == "hottest" || word == "maddest" || word == "saddest" || word == "slimmest" || word == "wettest" || word == "fattest"){
          ...
          let edSlice1Exceptions = ["adhered","axed","cited","cleansed","convened","continued","created","freed","guided","intrigued","owed","recreated","shared","typed"];
                        if (endsDExceptions.includes(word)) {
                          ...
                          possible_lemma = word.slice(0,-1);
                        } else if (word == "aimed" || word == "added" || word == "developed" || word == "focused" || word == "interpreted" || word == "misinterpreted" || (len(word, -5) == "o" && len(word, -4) == "a") || (len(word, -5) == "e" && len(word, -4) == "a") || len(word, -3) == "x" || (len(word, -4) == "e" && len(word, -3) == "n") || (len(word, -4) == "n" && len(word, -3) == "g")){
                            possible_lemma = word.slice(0,-2);
                        } else if (len(word, -3) == "i") {
                            possible_lemma = word.slice(0,-3) + "y";
                           ...
                           possible_lemma = word.slice(0,-2);
                            ...
                            if (word == "acquired" || word == "persuaded" || word == "required" || word == "quoted" || word == "excited"){
                              ...
                              possible_lemma = word.slice(0, -2);
                            } else if (len(word, -3) == "t" && len(word, -4) == "i"){
                              possible_lemma = word.slice(0, -2);
          ...
          let leaveAloneExceptions = ['boing', 'bring', 'ceiling', 'during', 'evening', 'king',  'ongoing', 'outing', 'outgoing', 'offspring', 'outstanding', 'ring', 'sing', 'sling', 'spring', 'handspring', 'wellspring', 'sting', 'string', 'hamstring', 'drawstring', 'heartstring', 'shoestring', 'swing','thing', 'well-being', 'wing', 'wring']
                      if (leaveAloneExceptions.includes(word)) {
                        ...
                        possible_lemma = word;
                      } else if (oringBackExceptions.includes(word) || len(word, -6) == "e" && len(word,-5) == "a" && len(word, -4) == "s" || (len(word,-5) == "y" && len(word, -4) == "p") || (len(word,-5) == "r" && len(word, -4) == "c") || (len(word,-5) == "p" && len(word, -4) == "s" || word == "challenging")){
                        possible_lemma = word.slice(0, -3) + "e";
                      } else if (word.substr((word.length - 5)) == "thing") {
                          possible_lemma = word; 
                        } else if (word == "adding" || word == "asking" || word == "allowing" || word == "aiming" || word == "developing" || word == "waxing" || word == "focusing" || word =="freeing" || word =="interpreting" || word =="misinterpreting" || len(word, -6) == "e" && len(word, -5) == "a" || len(word, -6) == "o" && len(word, -5) == "a" || len(word, -4) == "x" || (len(word, -5) == "e" && len(word, -4) == "n") || (len(word, -5) == "n" && len(word, -4) == "g")) {
                            possible_lemma = word.slice(0, -3);
                          ...
                          if (len(word, -5) == "s" || len(word, -5) == "l" || word == "erring") {
                            ...
                            if (vowel.includes(len(word, -5))) {
                               if (len(word, -6) == len(word, -5)){
                                  ...
                                  possible_lemma = word.slice(0, -3) + "e";
                                } else if (len(word, -4) == "r" && vowel.includes(len(word, -5)) || len(word, -6) == len(word, -7) || (len(word, -4) == "t" && len(word, -5) == "i")){
                                  possible_lemma = word.slice(0, -3);
